    ...
    const result = Service.of({
      ...
      get: Effect.fn("V2Session.get")(function* (sessionID) {
        const row = yield* Effect.promise(() =>
          Database.use((db) => db.select().from(SessionTable).where(eq(SessionTable.id, sessionID)).then((rows) => rows[0])),
        )
      ...
      list: Effect.fn("V2Session.list")(function* (input) {
        ...
        const rows = yield* Effect.promise(() => (input.limit === undefined ? query : query.limit(input.limit)))
