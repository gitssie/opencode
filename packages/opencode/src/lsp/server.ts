...
export namespace LSPServer {
  ...
  export const Typescript: Info = {
    ...
    async spawn(root) {
      ...
      const vue = Module.resolve("vue", root)
      let vuePluginPath: string | undefined
      if (vue) {
        vuePluginPath = Module.resolve("@vue/typescript-plugin", Global.Path.bin)
        if (!vuePluginPath) {
          if (!Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) {
            log.info("installing @vue/language-server for typescript-plugin")
            await Npm.which("@vue/language-server")
          }
          vuePluginPath = Module.resolve("@vue/typescript-plugin", Global.Path.bin)
        }
      }
    ...
    async setup({ connection, getClients, initialize }) {
      ...
      return {
        ...
        documentSymbol: async (input: { path: string }) => {
          ...
          const content = await fs.readFile(input.path, "utf8").catch(() => "")
  ...
  export const ESLint: Info = {
    ...
    async spawn(root) {
      ...
      log.info("spawning eslint server", { serverPath })
      ...
      return {
        ready: () =>
          new Promise<void>((resolve) => {
            ...
            connection.onNotification("window/logMessage", (params: any) => {
              const message = (params.message ?? "").toLowerCase()
              if (message.includes("ESLint") || message.includes("ready")) {
                clearTimeout(timeout)
                resolve()
              }
            })
  ...
  export const JDTLS: Info = {
    ...
    root: async (file) => {
      ...
      for await (const item of files) found = item
